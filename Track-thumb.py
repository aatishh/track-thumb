...
# 'Sketch4': 32 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_3) as sk_Sketch4_3:
    with BuildLine():
        ...
        # Near-straight arc (sagitta=0.008367mm) replaced with Line
...
# 'Sketch3': 75 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_6) as sk_Sketch3_6:
    with BuildLine():
        ...
        # Near-straight arc (sagitta=0.009441mm) replaced with Line
        ...
        # Near-straight arc (sagitta=0.003116mm) replaced with Line
...
with BuildSketch(_inclined_plane_7) as sk_Sketch10_7:
...
with BuildSketch(_inclined_plane_8) as sk_Sketch11_8:
    with Locations((94.2326, -36.6264)):
        Circle(radius=3.0)

# 'Sketch13': 24 segments → Line/RadiusArc profile
...
# 'Sketch12': 36 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_10) as sk_Sketch12_10:
    with BuildLine():
        ...
        # Near-straight arc (sagitta=0.003818mm) replaced with Line
...
# 'Sketch17': 7 segments → Line/RadiusArc profile
...
# NOTE: loft 'Loft1' uses unified point-list profiles
# (at least one profile is on an inclined plane — all profiles sampled to point lists
#  to match edge counts and prevent loft twisting).
# Resample count: N=64  [override]
# Loft profile 0: 'Sketch16'
# Profile stats: orig_pts=5, resampled_to=64, sharp_corners=5, dedup_removed=0, winding=was_CW_flipped
...
with BuildSketch(_inclined_plane_14) as sk_Sketch19_14:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch19_14 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch19_14 = Wire.combine(_inc_edges_sk_Sketch19_14)[0]
_wire_sk_Sketch19_14 = _wire_sk_Sketch19_14.moved(_inclined_plane_14.location)
_mkf_sk_Sketch19_14 = BRepBuilderAPI_MakeFace(_inclined_plane_14.wrapped, _wire_sk_Sketch19_14.wrapped, True)
_face_sk_Sketch19_14 = Face(_mkf_sk_Sketch19_14.Face())

# 'Sketch20': circle on inclined plane
_inclined_plane_15 = Plane(
    origin=Vector(-30.5781, -0.0167, 52.9635),
    x_dir=Vector(0.000545, -1.0, 0.0),
    z_dir=Vector(-0.499995, -0.000273, 0.866028),
)
with BuildSketch(_inclined_plane_15) as sk_Sketch20_15:
    with Locations((-13.1288, -26.7932)):
        Circle(radius=21.0006)

_solid2_sk_Sketch20_15 = extrude(sk_Sketch20_15.sketch, amount=-5.0, dir=Vector(-0.499995, -0.000273, 0.866028)).solid()
...
# 'Sketch22': 8 segments → revolve profile
with BuildSketch(_inclined_plane_16) as sk_Sketch22_15:
    with BuildLine():
    ...
    make_face()
# 'Sketch23': 18 segments → Line/RadiusArc profile
_inclined_plane_17 = Plane(
    origin=Vector(-30.5781, -0.0167, 52.9635),
    x_dir=Vector(0.000545, -1.0, 0.0),
    z_dir=Vector(-0.499995, -0.000273, 0.866028),
)
with BuildSketch(_inclined_plane_17) as sk_Sketch23_17:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch23_17 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch23_17 = Wire.combine(_inc_edges_sk_Sketch23_17)[0]
_wire_sk_Sketch23_17 = _wire_sk_Sketch23_17.moved(_inclined_plane_17.location)
_mkf_sk_Sketch23_17 = BRepBuilderAPI_MakeFace(_inclined_plane_17.wrapped, _wire_sk_Sketch23_17.wrapped, True)
_face_sk_Sketch23_17 = Face(_mkf_sk_Sketch23_17.Face())

# 'Sketch24': 12 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_18) as sk_Sketch24_18:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch24_18 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch24_18 = Wire.combine(_inc_edges_sk_Sketch24_18)[0]
_wire_sk_Sketch24_18 = _wire_sk_Sketch24_18.moved(_inclined_plane_18.location)
_mkf_sk_Sketch24_18 = BRepBuilderAPI_MakeFace(_inclined_plane_18.wrapped, _wire_sk_Sketch24_18.wrapped, True)
_face_sk_Sketch24_18 = Face(_mkf_sk_Sketch24_18.Face())

# 'Sketch31': 4 segments → Line/RadiusArc profile
...
# 'Sketch32': 4 segments → Line/RadiusArc profile
...
# 'Sketch33': 4 segments → Line/RadiusArc profile
_inclined_plane_21 = Plane(
    origin=Vector(-23.0829, -18.9806, 47.6387),
    x_dir=Vector(-0.635133, 0.772403, 0.0),
    z_dir=Vector(0.410461, 0.337515, -0.847116),
)
with BuildSketch(_inclined_plane_21) as sk_Sketch33_21:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch33_21 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch33_21 = Wire.combine(_inc_edges_sk_Sketch33_21)[0]
_wire_sk_Sketch33_21 = _wire_sk_Sketch33_21.moved(_inclined_plane_21.location)
_mkf_sk_Sketch33_21 = BRepBuilderAPI_MakeFace(_inclined_plane_21.wrapped, _wire_sk_Sketch33_21.wrapped, True)
_face_sk_Sketch33_21 = Face(_mkf_sk_Sketch33_21.Face())

# 'Sketch34': 3 segments → Line/RadiusArc profile
_inclined_plane_22 = Plane(
    origin=Vector(-18.5737, -18.4742, 47.5662),
    x_dir=Vector(0.705203, -0.709006, -0.0),
    z_dir=Vector(-0.342038, -0.340204, 0.87594),
)
with BuildSketch(_inclined_plane_22) as sk_Sketch34_22:
    with BuildLine():
        Line((-77.9886, -36.5671), (-78.1814, -37.9863))
        Line((-78.1814, -37.9863), (-80.6308, -36.2027))
        Line((-80.6308, -36.2027), (-77.9886, -36.5671))
    _inc_edges_sk_Sketch34_22 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch34_22 = Wire.combine(_inc_edges_sk_Sketch34_22)[0]
_wire_sk_Sketch34_22 = _wire_sk_Sketch34_22.moved(_inclined_plane_22.location)
_mkf_sk_Sketch34_22 = BRepBuilderAPI_MakeFace(_inclined_plane_22.wrapped, _wire_sk_Sketch34_22.wrapped, True)
_face_sk_Sketch34_22 = Face(_mkf_sk_Sketch34_22.Face())

# 'Sketch35': 4 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_23) as sk_Sketch35_23:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch35_23 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch35_23 = Wire.combine(_inc_edges_sk_Sketch35_23)[0]
_wire_sk_Sketch35_23 = _wire_sk_Sketch35_23.moved(_inclined_plane_23.location)
_mkf_sk_Sketch35_23 = BRepBuilderAPI_MakeFace(_inclined_plane_23.wrapped, _wire_sk_Sketch35_23.wrapped, True)
_face_sk_Sketch35_23 = Face(_mkf_sk_Sketch35_23.Face())

# 'Sketch37': 5 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_24) as sk_Sketch37_24:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch37_24 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch37_24 = Wire.combine(_inc_edges_sk_Sketch37_24)[0]
_wire_sk_Sketch37_24 = _wire_sk_Sketch37_24.moved(_inclined_plane_24.location)
_mkf_sk_Sketch37_24 = BRepBuilderAPI_MakeFace(_inclined_plane_24.wrapped, _wire_sk_Sketch37_24.wrapped, True)
_face_sk_Sketch37_24 = Face(_mkf_sk_Sketch37_24.Face())

# 'Sketch38': 4 segments → Line/RadiusArc profile
_inclined_plane_25 = Plane(
    origin=Vector(-24.5642, -20.9302, 50.3967),
    x_dir=Vector(0.648559, -0.761165, -0.0),
    z_dir=Vector(-0.410471, -0.349747, 0.842135),
)
with BuildSketch(_inclined_plane_25) as sk_Sketch38_25:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch38_25 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch38_25 = Wire.combine(_inc_edges_sk_Sketch38_25)[0]
_wire_sk_Sketch38_25 = _wire_sk_Sketch38_25.moved(_inclined_plane_25.location)
_mkf_sk_Sketch38_25 = BRepBuilderAPI_MakeFace(_inclined_plane_25.wrapped, _wire_sk_Sketch38_25.wrapped, True)
_face_sk_Sketch38_25 = Face(_mkf_sk_Sketch38_25.Face())
...
# 'Sketch41': 14 segments -> sweep profile
with BuildSketch(_plane_Sweep2) as sk_Sketch41_25:
    with BuildLine():
        ...
        RadiusArc((-40.1626, -68.7295), (-39.9557, -69.0031), -0.8905)
        Line((-39.9557, -69.0031), (-39.9596, -69.2548))
        # Near-straight arc (sagitta=0.007172mm) replaced with Line
        Line((-39.9596, -69.2548), (-39.2829, -69.3553))
        Line((-39.2829, -69.3553), (-39.0071, -69.3488))
        Line((-39.0071, -69.3488), (-18.5289, -69.3739))
        Line((-18.5289, -69.3739), (-16.5218, -69.3761))
        RadiusArc((-16.5218, -69.3761), (-14.3571, -68.6339), -3.2675)
        RadiusArc((-14.3571, -68.6339), (-13.4299, -66.9598), -2.5131)
        Line((-13.4299, -66.9598), (-12.6322, -63.184))
        Line((-12.6322, -63.184), (-26.4996, -63.1673))
        Line((-26.4996, -63.1673), (-40.8171, -63.1495))
        Line((-40.8171, -63.1495), (-40.2927, -68.316))
    make_face()
# NOTE: loft 'Loft7' uses unified point-list profiles
# (at least one profile is on an inclined plane — all profiles sampled to point lists
#  to match edge counts and prevent loft twisting).
# Resample count: N=64  [override]
# Loft profile 0: 'Sketch53'
    # Profile 'Sketch53' was CW; reversed to CCW for loft
# Profile stats: orig_pts=10, resampled_to=64, sharp_corners=8, dedup_removed=0, winding=was_CW_flipped
...
# Loft profile 1: 'Sketch52'
    # Profile 'Sketch52' was CW; reversed to CCW for loft
# Profile stats: orig_pts=5, resampled_to=64, sharp_corners=5, dedup_removed=0, winding=was_CW_flipped
...
# 'Sketch58': 7 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_29) as sk_Sketch58_29:
    with BuildLine():
        RadiusArc((-16.1895, -69.3133), (-14.4998, -71.5), -19.0465)
        # Near-straight arc (sagitta=0.00791mm) replaced with Line
    ...
    _inc_edges_sk_Sketch58_29 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch58_29 = Wire.combine(_inc_edges_sk_Sketch58_29)[0]
_wire_sk_Sketch58_29 = _wire_sk_Sketch58_29.moved(_inclined_plane_29.location)
_mkf_sk_Sketch58_29 = BRepBuilderAPI_MakeFace(_inclined_plane_29.wrapped, _wire_sk_Sketch58_29.wrapped, True)
_face_sk_Sketch58_29 = Face(_mkf_sk_Sketch58_29.Face())

# 'Sketch59': 7 segments → Line/RadiusArc profile
_inclined_plane_30 = Plane(
    origin=Vector(-97.745, -7.0104, 0.0),
    x_dir=Vector(0.0, 0.0, 1.0),
    z_dir=Vector(-0.997438, -0.071538, 0.0),
)
with BuildSketch(_inclined_plane_30) as sk_Sketch59_30:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch59_30 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch59_30 = Wire.combine(_inc_edges_sk_Sketch59_30)[0]
_wire_sk_Sketch59_30 = _wire_sk_Sketch59_30.moved(_inclined_plane_30.location)
_mkf_sk_Sketch59_30 = BRepBuilderAPI_MakeFace(_inclined_plane_30.wrapped, _wire_sk_Sketch59_30.wrapped, True)
_face_sk_Sketch59_30 = Face(_mkf_sk_Sketch59_30.Face())

# 'Sketch60': 6 segments → Line/RadiusArc profile
_inclined_plane_31 = Plane(
    origin=Vector(-103.4562, 19.102, 0.0),
    x_dir=Vector(0.0, 0.0, 1.0),
    z_dir=Vector(-0.983378, 0.18157, 0.0),
)
with BuildSketch(_inclined_plane_31) as sk_Sketch60_31:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch60_31 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch60_31 = Wire.combine(_inc_edges_sk_Sketch60_31)[0]
_wire_sk_Sketch60_31 = _wire_sk_Sketch60_31.moved(_inclined_plane_31.location)
_mkf_sk_Sketch60_31 = BRepBuilderAPI_MakeFace(_inclined_plane_31.wrapped, _wire_sk_Sketch60_31.wrapped, True)
_face_sk_Sketch60_31 = Face(_mkf_sk_Sketch60_31.Face())

# 'Sketch61': 4 segments → Line/RadiusArc profile
_inclined_plane_32 = Plane(
    origin=Vector(-102.5633, 6.6616, 0.0),
    x_dir=Vector(0.0, 0.0, 1.0),
    z_dir=Vector(-0.997897, 0.064814, 0.0),
)
with BuildSketch(_inclined_plane_32) as sk_Sketch61_32:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch61_32 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch61_32 = Wire.combine(_inc_edges_sk_Sketch61_32)[0]
_wire_sk_Sketch61_32 = _wire_sk_Sketch61_32.moved(_inclined_plane_32.location)
_mkf_sk_Sketch61_32 = BRepBuilderAPI_MakeFace(_inclined_plane_32.wrapped, _wire_sk_Sketch61_32.wrapped, True)
_face_sk_Sketch61_32 = Face(_mkf_sk_Sketch61_32.Face())

# 'Sketch62': 4 segments → Line/RadiusArc profile
_inclined_plane_33 = Plane(
    origin=Vector(-102.5633, 6.6616, 0.0),
    x_dir=Vector(-0.064814, -0.997897, 0.0),
    z_dir=Vector(-0.997897, 0.064814, 0.0),
)
with BuildSketch(_inclined_plane_33) as sk_Sketch62_33:
    with BuildLine():
        RadiusArc((-27.9967, 20.8348), (-26.7079, 21.3804), -1.9197)
        # Near-straight arc (sagitta=0.001065mm) replaced with Line
        Line((-26.7079, 21.3804), (-26.9835, 22.0959))
        RadiusArc((-26.9835, 22.0959), (-28.0418, 20.8049), 2.9475)
        Line((-28.0418, 20.8049), (-27.9967, 20.8348))
    _inc_edges_sk_Sketch62_33 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch62_33 = Wire.combine(_inc_edges_sk_Sketch62_33)[0]
_wire_sk_Sketch62_33 = _wire_sk_Sketch62_33.moved(_inclined_plane_33.location)
_mkf_sk_Sketch62_33 = BRepBuilderAPI_MakeFace(_inclined_plane_33.wrapped, _wire_sk_Sketch62_33.wrapped, True)
_face_sk_Sketch62_33 = Face(_mkf_sk_Sketch62_33.Face())

# 'Sketch63': 5 segments → Line/RadiusArc profile
_inclined_plane_34 = Plane(
    origin=Vector(-89.5885, 48.7912, 0.0),
    x_dir=Vector(0.0, 0.0, -1.0),
    z_dir=Vector(0.878205, -0.478284, 0.0),
)
with BuildSketch(_inclined_plane_34) as sk_Sketch63_34:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch63_34 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch63_34 = Wire.combine(_inc_edges_sk_Sketch63_34)[0]
_wire_sk_Sketch63_34 = _wire_sk_Sketch63_34.moved(_inclined_plane_34.location)
_mkf_sk_Sketch63_34 = BRepBuilderAPI_MakeFace(_inclined_plane_34.wrapped, _wire_sk_Sketch63_34.wrapped, True)
_face_sk_Sketch63_34 = Face(_mkf_sk_Sketch63_34.Face())

# 'Sketch66': 4 segments → Line/RadiusArc profile
_inclined_plane_35 = Plane(
    origin=Vector(-98.4039, 33.7717, 0.0),
    x_dir=Vector(0.0, 0.0, -1.0),
    z_dir=Vector(0.945848, -0.32461, 0.0),
)
with BuildSketch(_inclined_plane_35) as sk_Sketch66_35:
    with BuildLine():
        Line((-10.5376, -2.4503), (-10.5531, -3.5482))
        Line((-10.5531, -3.5482), (-21.2, -3.2557))
        Line((-21.2, -3.2557), (-21.2, -2.3))
        Line((-21.2, -2.3), (-10.5376, -2.4503))
    _inc_edges_sk_Sketch66_35 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch66_35 = Wire.combine(_inc_edges_sk_Sketch66_35)[0]
_wire_sk_Sketch66_35 = _wire_sk_Sketch66_35.moved(_inclined_plane_35.location)
_mkf_sk_Sketch66_35 = BRepBuilderAPI_MakeFace(_inclined_plane_35.wrapped, _wire_sk_Sketch66_35.wrapped, True)
_face_sk_Sketch66_35 = Face(_mkf_sk_Sketch66_35.Face())

# 'Sketch67': 4 segments → Line/RadiusArc profile
_inclined_plane_36 = Plane(
    origin=Vector(0.0, 0.0, 6.9074),
    x_dir=Vector(1.0, 0.0, 0.0),
    z_dir=Vector(0.0, 0.0, -1.0),
)
with BuildSketch(_inclined_plane_36) as sk_Sketch67_36:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch67_36 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch67_36 = Wire.combine(_inc_edges_sk_Sketch67_36)[0]
_wire_sk_Sketch67_36 = _wire_sk_Sketch67_36.moved(_inclined_plane_36.location)
_mkf_sk_Sketch67_36 = BRepBuilderAPI_MakeFace(_inclined_plane_36.wrapped, _wire_sk_Sketch67_36.wrapped, True)
_face_sk_Sketch67_36 = Face(_mkf_sk_Sketch67_36.Face())

# 'Sketch69': 4 segments → Line/RadiusArc profile
_inclined_plane_37 = Plane(
    origin=Vector(-55.7112, -25.6125, -4.5588),
    x_dir=Vector(0.363349, -0.855857, 0.368086),
    z_dir=Vector(0.90608, 0.416559, 0.074145),
)
with BuildSketch(_inclined_plane_37) as sk_Sketch69_37:
    with BuildLine():
        RadiusArc((-14.4751, -18.2396), (-5.6735, -33.6475), -32.3482)
        RadiusArc((-5.6735, -33.6475), (-0.0687, -32.3275), -9.5164)
        RadiusArc((-0.0687, -32.3275), (-1.081, -12.7235), -42.7406)
        RadiusArc((-1.081, -12.7235), (-14.4751, -18.2396), -72.9661)
    _inc_edges_sk_Sketch69_37 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch69_37 = Wire.combine(_inc_edges_sk_Sketch69_37)[0]
_wire_sk_Sketch69_37 = _wire_sk_Sketch69_37.moved(_inclined_plane_37.location)
_mkf_sk_Sketch69_37 = BRepBuilderAPI_MakeFace(_inclined_plane_37.wrapped, _wire_sk_Sketch69_37.wrapped, True)
_face_sk_Sketch69_37 = Face(_mkf_sk_Sketch69_37.Face())

# 'Sketch70': 5 segments → Line/RadiusArc profile
_inclined_plane_38 = Plane(
    origin=Vector(-59.3198, 65.884, 0.0),
    x_dir=Vector(0.0, 0.0, -1.0),
    z_dir=Vector(0.669115, -0.743159, 0.0),
)
with BuildSketch(_inclined_plane_38) as sk_Sketch70_38:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch70_38 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch70_38 = Wire.combine(_inc_edges_sk_Sketch70_38)[0]
_wire_sk_Sketch70_38 = _wire_sk_Sketch70_38.moved(_inclined_plane_38.location)
_mkf_sk_Sketch70_38 = BRepBuilderAPI_MakeFace(_inclined_plane_38.wrapped, _wire_sk_Sketch70_38.wrapped, True)
_face_sk_Sketch70_38 = Face(_mkf_sk_Sketch70_38.Face())

# 'Sketch71': 6 segments → Line/RadiusArc profile
_inclined_plane_39 = Plane(
    origin=Vector(-115.8778, 16.4497, 0.0),
    x_dir=Vector(-0.0, 0.0, -1.0),
    z_dir=Vector(0.990074, -0.140548, -0.0),
)
with BuildSketch(_inclined_plane_39) as sk_Sketch71_39:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch71_39 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch71_39 = Wire.combine(_inc_edges_sk_Sketch71_39)[0]
_wire_sk_Sketch71_39 = _wire_sk_Sketch71_39.moved(_inclined_plane_39.location)
_mkf_sk_Sketch71_39 = BRepBuilderAPI_MakeFace(_inclined_plane_39.wrapped, _wire_sk_Sketch71_39.wrapped, True)
_face_sk_Sketch71_39 = Face(_mkf_sk_Sketch71_39.Face())

# 'Sketch72': 5 segments → Line/RadiusArc profile
_inclined_plane_40 = Plane(
    origin=Vector(-114.7258, -9.3256, 0.0),
    x_dir=Vector(0.0, 0.0, -1.0),
    z_dir=Vector(0.996713, 0.081019, 0.0),
)
with BuildSketch(_inclined_plane_40) as sk_Sketch72_40:
    with BuildLine():
        RadiusArc((-11.2, 21.7), (-11.1371, 21.2185), -0.7135)
        # Near-straight arc (sagitta=0.006894mm) replaced with Line
    ...
    _inc_edges_sk_Sketch72_40 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch72_40 = Wire.combine(_inc_edges_sk_Sketch72_40)[0]
_wire_sk_Sketch72_40 = _wire_sk_Sketch72_40.moved(_inclined_plane_40.location)
_mkf_sk_Sketch72_40 = BRepBuilderAPI_MakeFace(_inclined_plane_40.wrapped, _wire_sk_Sketch72_40.wrapped, True)
_face_sk_Sketch72_40 = Face(_mkf_sk_Sketch72_40.Face())

# 'Sketch73': 4 segments → Line/RadiusArc profile
_inclined_plane_41 = Plane(
    origin=Vector(0.0, 0.0, 10.9074),
    x_dir=Vector(-1.0, 0.0, 0.0),
    z_dir=Vector(0.0, 0.0, 1.0),
)
with BuildSketch(_inclined_plane_41) as sk_Sketch73_41:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch73_41 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch73_41 = Wire.combine(_inc_edges_sk_Sketch73_41)[0]
_wire_sk_Sketch73_41 = _wire_sk_Sketch73_41.moved(_inclined_plane_41.location)
_mkf_sk_Sketch73_41 = BRepBuilderAPI_MakeFace(_inclined_plane_41.wrapped, _wire_sk_Sketch73_41.wrapped, True)
_face_sk_Sketch73_41 = Face(_mkf_sk_Sketch73_41.Face())

# NOTE: loft 'Loft9' uses unified point-list profiles
# (at least one profile is on an inclined plane — all profiles sampled to point lists
#  to match edge counts and prevent loft twisting).
# Resample count: N=64  [override]
# Loft profile 0: 'Sketch77'
    # Profile 'Sketch77' was CW; reversed to CCW for loft
# Profile stats: orig_pts=15, resampled_to=64, sharp_corners=8, dedup_removed=0, winding=was_CW_flipped
...
# Loft profile 1: 'Sketch78'
    # Profile 'Sketch78' was CW; reversed to CCW for loft
# Profile stats: orig_pts=9, resampled_to=64, sharp_corners=6, dedup_removed=1, winding=was_CW_flipped
...
# NOTE: loft 'Loft11' uses unified point-list profiles
# (at least one profile is on an inclined plane — all profiles sampled to point lists
#  to match edge counts and prevent loft twisting).
# Resample count: N=64  [override]
# Loft profile 0: 'Sketch81'
    # Profile 'Sketch81' was CW; reversed to CCW for loft
# Profile stats: orig_pts=8, resampled_to=64, sharp_corners=6, dedup_removed=0, winding=was_CW_flipped
...
# 'Sketch84': 3 segments → Line/RadiusArc profile
_inclined_plane_46 = Plane(
    origin=Vector(-91.3238, 49.7362, 0.0),
    x_dir=Vector(0.478284, 0.878205, 0.0),
    z_dir=Vector(0.878205, -0.478284, -0.0),
)
with BuildSketch(_inclined_plane_46) as sk_Sketch84_46:
    with BuildLine():
        Line((-20.1326, 26.3484), (-20.315, 20.9074))
        Line((-20.315, 20.9074), (-23.7244, 20.9074))
        RadiusArc((-23.7244, 20.9074), (-20.1326, 26.3484), -15.0296)
    _inc_edges_sk_Sketch84_46 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch84_46 = Wire.combine(_inc_edges_sk_Sketch84_46)[0]
_wire_sk_Sketch84_46 = _wire_sk_Sketch84_46.moved(_inclined_plane_46.location)
_mkf_sk_Sketch84_46 = BRepBuilderAPI_MakeFace(_inclined_plane_46.wrapped, _wire_sk_Sketch84_46.wrapped, True)
_face_sk_Sketch84_46 = Face(_mkf_sk_Sketch84_46.Face())
...
# NOTE: loft 'Loft13' uses unified point-list profiles
# (at least one profile is on an inclined plane — all profiles sampled to point lists
#  to match edge counts and prevent loft twisting).
# Resample count: N=64  [override]
# Loft profile 0: 'Sketch83'
    # Profile 'Sketch83' was CW; reversed to CCW for loft
# Profile stats: orig_pts=6, resampled_to=64, sharp_corners=6, dedup_removed=0, winding=was_CW_flipped
...
# Loft profile 1: 'Sketch82'
    # Profile 'Sketch82' was CW; reversed to CCW for loft
# Profile stats: orig_pts=15, resampled_to=64, sharp_corners=4, dedup_removed=0, winding=was_CW_flipped
sk_Sketch82_loft_50_edges = [
    ...
    Edge.make_line((-35.6296, -8.6747, 0), (-35.5381, -9.3799, 0)),
    Edge.make_line((-35.5381, -9.3799, 0), (-35.3994, -10.0774, 0)),
    Edge.make_line((-35.3994, -10.0774, 0), (-35.2603, -10.7748, 0)),
    Edge.make_line((-35.2603, -10.7748, 0), (-35.1212, -11.4722, 0)),
    Edge.make_line((-35.1212, -11.4722, 0), (-34.6098, -11.906, 0)),
    Edge.make_line((-34.6098, -11.906, 0), (-33.9983, -12.269, 0)),
    Edge.make_line((-33.9983, -12.269, 0), (-33.3625, -12.5811, 0)),
    Edge.make_line((-33.3625, -12.5811, 0), (-32.6951, -12.8266, 0)),
    Edge.make_line((-32.6951, -12.8266, 0), (-32.0251, -13.0621, 0)),
    Edge.make_line((-32.0251, -13.0621, 0), (-31.3241, -13.1819, 0)),
    Edge.make_line((-31.3241, -13.1819, 0), (-30.6231, -13.3016, 0)),
    Edge.make_line((-30.6231, -13.3016, 0), (-29.9149, -13.3274, 0)),
    Edge.make_line((-29.9149, -13.3274, 0), (-29.2038, -13.3174, 0)),
    Edge.make_line((-29.2038, -13.3174, 0), (-28.4978, -13.2595, 0)),
    Edge.make_line((-28.4978, -13.2595, 0), (-27.8005, -13.1201, 0)),
    Edge.make_line((-27.8005, -13.1201, 0), (-27.1038, -12.9787, 0)),
    Edge.make_line((-27.1038, -12.9787, 0), (-26.4435, -12.7146, 0)),
    Edge.make_line((-26.4435, -12.7146, 0), (-25.7832, -12.4505, 0)),
    Edge.make_line((-25.7832, -12.4505, 0), (-25.1621, -12.1097, 0)),
    Edge.make_line((-25.1621, -12.1097, 0), (-24.561, -11.7297, 0)),
    Edge.make_line((-24.561, -11.7297, 0), (-24.2502, -11.2754, 0)),
    Edge.make_line((-24.2502, -11.2754, 0), (-24.5947, -10.6532, 0)),
    Edge.make_line((-24.5947, -10.6532, 0), (-24.9391, -10.0311, 0)),
    Edge.make_line((-24.9391, -10.0311, 0), (-25.2835, -9.4089, 0)),
    Edge.make_line((-25.2835, -9.4089, 0), (-25.6279, -8.7868, 0)),
    Edge.make_line((-25.6279, -8.7868, 0), (-25.9723, -8.1646, 0)),
    Edge.make_line((-25.9723, -8.1646, 0), (-26.3167, -7.5425, 0)),
    Edge.make_line((-26.3167, -7.5425, 0), (-26.6612, -6.9203, 0)),
    Edge.make_line((-26.6612, -6.9203, 0), (-27.0056, -6.2982, 0)),
    Edge.make_line((-27.0056, -6.2982, 0), (-27.35, -5.676, 0)),
    Edge.make_line((-27.35, -5.676, 0), (-27.6944, -5.0539, 0)),
    Edge.make_line((-27.6944, -5.0539, 0), (-28.0388, -4.4317, 0)),
    Edge.make_line((-28.0388, -4.4317, 0), (-28.3833, -3.8096, 0)),
    Edge.make_line((-28.3833, -3.8096, 0), (-28.7277, -3.1874, 0)),
    Edge.make_line((-28.7277, -3.1874, 0), (-29.0721, -2.5652, 0)),
    Edge.make_line((-29.0721, -2.5652, 0), (-29.4165, -1.9431, 0)),
    Edge.make_line((-29.4165, -1.9431, 0), (-29.7609, -1.3209, 0)),
    Edge.make_line((-29.7609, -1.3209, 0), (-30.1053, -0.6988, 0)),
    Edge.make_line((-30.1053, -0.6988, 0), (-30.4498, -0.0766, 0)),
    Edge.make_line((-30.4498, -0.0766, 0), (-30.7942, 0.5455, 0)),
    Edge.make_line((-30.7942, 0.5455, 0), (-31.1386, 1.1677, 0)),
    Edge.make_line((-31.1386, 1.1677, 0), (-31.483, 1.7898, 0)),
    Edge.make_line((-31.483, 1.7898, 0), (-31.8274, 2.412, 0)),
    Edge.make_line((-31.8274, 2.412, 0), (-32.1718, 3.0341, 0)),
    Edge.make_line((-32.1718, 3.0341, 0), (-32.7912, 2.9207, 0)),
    Edge.make_line((-32.7912, 2.9207, 0), (-33.4592, 2.6769, 0)),
    Edge.make_line((-33.4592, 2.6769, 0), (-34.1272, 2.4332, 0)),
...
sk_Sketch82_loft_50_face = Plane(origin=Vector(-66.1234, -22.9045, 21.454), x_dir=Vector(0.32731, -0.944917, 0.0), z_dir=Vector(-0.903413, -0.312934, 0.293115)) * sk_Sketch82_loft_50_face
...
# 'Sketch93': 4 segments -> sweep profile
with BuildSketch(_plane_Sweep3) as sk_Sketch93_50:
    ...
    make_face()
# 'Sketch95': 4 segments → Line/RadiusArc profile
_inclined_plane_52 = Plane(
    origin=Vector(-24.7304, -20.3355, 51.0393),
    x_dir=Vector(0.635133, -0.772403, -0.0),
    z_dir=Vector(-0.410461, -0.337515, 0.847116),
)
with BuildSketch(_inclined_plane_52) as sk_Sketch95_52:
    with BuildLine():
    ...
    _inc_edges_sk_Sketch95_52 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch95_52 = Wire.combine(_inc_edges_sk_Sketch95_52)[0]
_wire_sk_Sketch95_52 = _wire_sk_Sketch95_52.moved(_inclined_plane_52.location)
_mkf_sk_Sketch95_52 = BRepBuilderAPI_MakeFace(_inclined_plane_52.wrapped, _wire_sk_Sketch95_52.wrapped, True)
_face_sk_Sketch95_52 = Face(_mkf_sk_Sketch95_52.Face())

# NOTE: loft 'MyLoft1' uses unified point-list profiles
# (at least one profile is on an inclined plane — all profiles sampled to point lists
#  to match edge counts and prevent loft twisting).
# Resample count: N=64  [override]
# Loft profile 0: 'loft1'
    # Profile 'loft1' was CW; reversed to CCW for loft
# Profile stats: orig_pts=8, resampled_to=64, sharp_corners=7, dedup_removed=0, winding=was_CW_flipped
sk_loft1_loft_53_edges = [
...
sk_loft1_loft_53_wire = Wire(sk_loft1_loft_53_edges)
sk_loft1_loft_53_face = Face(sk_loft1_loft_53_wire)
sk_loft1_loft_53_face = Plane(origin=Vector(0.0, 0.0, 10.9074), x_dir=Vector(-1.0, 0.0, 0.0), z_dir=Vector(0.0, 0.0, 1.0)) * sk_loft1_loft_53_face


with BuildSketch(Plane.XY) as sk_loft2_loft_54:
    pass  # TODO: profile 'loft2' had fewer than 3 usable points


with BuildSketch(Plane.XY) as sk_loft3_loft_55:
    pass  # TODO: profile 'loft3' had fewer than 3 usable points
...
with BuildPart() as part:
    ...
    add(_solid)
    # Fusion depth expression: -4.00 mm
    ...
    # Fusion depth expression: -5.00 mm
    ...
    # v17.7: symmetric second direction
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # v17.7: two-sided second direction (inclined segments)
    ...
    # v17.7: two-sided second direction (inclined plane)
    add(_solid2_sk_Sketch20_15, mode=Mode.SUBTRACT)
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: -5.996 mm
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: -5.988 mm
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: 3.20 mm
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: 1.435 mm
    ...
    # Fusion depth expression: 1.00 mm
    ...
    # Fusion depth expression: 0.60 mm
    ...
    # Fusion depth expression: 2.00 mm
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: -1.50 mm
    ...
    # Fusion depth expression: 3.00 mm
    ...
    _vec = Vector(0.90608, 0.416559, 0.074145) * -21.0
    _solid = Solid.extrude(_face, _vec)
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: -1.912 mm
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: -1.942 mm
    ...
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: -2.029 mm
    ...
    # -- Extrude61 --
    _face = _face_sk_Sketch95_52
    _vec = Vector(-0.410461, -0.337515, 0.847116) * 2.0
    _solid = Solid.extrude(_face, _vec)
    _result_Extrude61 = cut_solids(part.part, _solid)
    if _result_Extrude61 is not None: part.part = _result_Extrude61

export_stl(part.part,  'Track-Thumb.stl')
